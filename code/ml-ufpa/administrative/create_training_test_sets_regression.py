...
Assume regression instead of classification.
Aldebaro. 2022
'''
import csv
...
Ntr = 3
Nva = 4
space_dimension = 3 #in fact both input and output dimensions is 2

seed(211) #random seed. It will change during class
...
def write_examples_in_csvwriter(output_classe, min_x, max_x, num_examples, space_dimension, csvwriter):
    for i in range(num_examples):
        ...
        for j in range(space_dimension):
            if j==0:
                output_row.append(1.2*randint(min_x, max_x))
            else:
