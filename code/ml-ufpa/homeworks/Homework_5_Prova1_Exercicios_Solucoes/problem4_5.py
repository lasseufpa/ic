...
def load_iris_first_2_features_and_binary():
    ...
    y = np.array(iris.target < 1).astype(int)
...
svc_with_linear_kernel = svm.SVC(kernel='linear', C=C, verbose=1, shrinking=True)
linear_svc = svm.LinearSVC(C=C, max_iter=100000, dual=True, tol=1e-10)
rbf_svm = svm.SVC(kernel='rbf', gamma=0.7, C=C)
polynomial_svm = svm.SVC(kernel='poly', degree=3, gamma='auto', C=C)
...
#Plot:
models = (svc_with_linear_kernel, linear_svc, rbf_svm, polynomial_svm)
